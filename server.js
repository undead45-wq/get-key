...
app.post('/generate_key.php', (req, res) => {
    ...
    db.run('INSERT INTO keys (key_code, expiry, days) VALUES (?, ?, ?)', 
        [key, expiryStr, days], function(err) {
        if (err) {
...
app.post('/api.php', (req, res) => {
    ...
    if (action === 'login') {
        ...
        db.get('SELECT * FROM keys WHERE key_code = ?', [key], function(err, row) {
            if (!row) {
...
<body>
    ...
    <script>
        ...
        function updateProgress() {
            ...
            if (tasksCompleted[1]) completedCount++;
            if (tasksCompleted[2]) completedCount++;
            if (tasksCompleted[3]) completedCount++;
            
            var percentage = (completedCount / 3) * 100;
        ...
        function startTaskTimer(taskId, button) {
            ...
            timers[taskId] = setTimeout(function() {
                clearInterval(timerInterval);
            }, 3000);
        }

        function openTask(taskId) {
            var url = taskUrls[taskId];
            window.open(url, '_blank');
            
            var button = document.getElementById('btn' + taskId);
            var statusEl = document.getElementById('status' + taskId);
        ...
        document.getElementById('btn1').addEventListener('click', function() { openTask(1); });
        document.getElementById('btn2').addEventListener('click', function() { openTask(2); });
        document.getElementById('btn3').addEventListener('click', function() { openTask(3); });
        ...
        document.getElementById('generateBtn').addEventListener('click', async function() {
            ...
            try {
                var response = await fetch('/generate_key.php', {
                    method: 'POST',
                    headers: {
